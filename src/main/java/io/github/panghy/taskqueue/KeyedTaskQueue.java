...
public class KeyedTaskQueue<K, T> implements TaskQueue<K, T> {
  ...
  private final LongUpDownCounter unclaimedQueueSize;
  private final LongUpDownCounter claimedQueueSize;
  ...
  private KeyedTaskQueue(
    ...
    this.unclaimedQueueSize = meter.upDownCounterBuilder("taskqueue.queue.unclaimed.size")
        .setDescription("Current number of unclaimed tasks in the queue")
        .setUnit("tasks")
        .build();

    this.claimedQueueSize = meter.upDownCounterBuilder("taskqueue.queue.claimed.size")
        .setDescription("Current number of claimed tasks in the queue")
        .setUnit("tasks")
        .build();
  }
  ...
  private void checkNotClosed() {
    if (closed) {
      throw new TaskQueueException("Queue is closed");
    }
  }

  /**
   * Wraps a watch future so that it also fails if the queue is closed.
   * Returns a future that completes when either the watch triggers or the queue is closed
   * (in which case it completes exceptionally).
   */
  private CompletableFuture<Void> raceWithClose(CompletableFuture<Void> watchF) {
    CompletableFuture<Void> raced = new CompletableFuture<>();
    watchF.whenComplete((v, ex) -> {
      if (ex != null) {
  ...
  public CompletableFuture<TaskKeyMetadata> enqueue(Transaction tr, K taskKey, T task, Duration delay, Duration ttl) {
    ...
    if (taskKeyB.length == 0) {
      throw new IllegalArgumentException("taskKey cannot be serialized to an empty byte array");
    }
    ...
    return taskF.thenApply(v -> {
          LOGGER.debug("Enqueued task: {}", describeTask(taskUuid, task));
    ...
    return taskMetadataF
        .thenCompose(taskMetadataProto -> {
          ...
          if (taskClaim.taskProto().getTaskVersion() == taskMetadataProto.getHighestVersionSeen()) {
            ...
            newerVersionScheduled.set(true);
    ...
    return taskMetadataF
        .thenCompose(taskMetadataProto -> {
          ...
          if (taskClaim.taskProto().getTaskVersion() == taskMetadataProto.getHighestVersionSeen()) {
            ...
            rescheduledToUnclaimed.set(true);
            LOGGER.info(
                "Task {} is not the latest version: {} != {}. Skipping to latest version.",
                describeTask(taskUuid, taskClaim.task()),
                taskClaim.taskProto().getTaskVersion(),
                taskMetadataProto.getHighestVersionSeen());
            var latestTaskKeyF =
  ...
  private CompletableFuture<Optional<TaskClaim<K, T>>> findUnclaimedTask(Transaction tr) {
    ...
    return taskKVF.thenCompose(taskKV -> {
      ...
      return allOf(taskKeyF, taskKeyMetadataF).thenApply(v -> {
        ...
        LOGGER.debug("Found unclaimed task: {}", describeTask(taskUuid, taskObj));
        ...
        if (taskProto.getAttempts() >= config.getMaxAttempts()) {
          ...
          LOGGER.warn(
              ...
              taskKeyMetadataProto.getHighestVersionSeen());
          attempts = 0;
        }
        attempts++;
  ...
  public CompletableFuture<Boolean> hasClaimedTasks(Transaction tr) {
    return tr.getRange(claimedTasks.range(), 1).asList().thenApply(kvs -> {
